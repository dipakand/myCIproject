<div class="row">
    ...
    <div class="col-md-12 <?php if((!isset($frmdate)) && (!isset($todate))) { echo "hidden";} ?>">&nbsp;
        <div class="col-sm-10 col-sm-offset-1 text-center alert alert-success" style="color:#fff;">&nbsp;
            <b>Purchase Order for The Duration of Date From : <?php echo $frmdate;?> - To : <?php echo $todate;?></b>
            <a onclick="window.open('<?php echo site_url('PurchaseController/excel_summary/'.$frmdate.'/'.$todate); ;?>')"  class="btn btn-primary pull-right">Excel sheet</a>
        </div>

        <div class="invoice-box" style="height:;">&nbsp;
            <?php
            //$select="select * from purchase_order where received!='' and status!=1 and recev_status!=0 and date between '".date("Y-m-d",strtotime($frmdate))."' and '".date("Y-m-d",strtotime($todate))."' order by date asc";
            ...
            <form method="post" enctype="multipart/form-data">
                ...
                <?php 
                 /*if($purchase->cod_percent > 0 )
                        {?>
                        <div style="text-align:left; margin-left:1%; font-size:11px;"><b>If payment done of cash. Please Pay <i class="fa fa-inr"></i> <?php //echo round($purchase->cod_amt,2); ?> /-.</b></div>
                        <?php
                        }  */       
                ?>
