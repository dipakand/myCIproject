<?php //print_r($vendor_row);?>
<div class="row">
    <div class="col-lg-12">
        <?php $this->load->view('PurchaseOrder/tabs');?>
        <div class="col-md-12 ">&nbsp;
            <?php if(!$this->session->userdata('vendor_data')){?>
            <form method="post">                                 
                <div class="col-md-12 ">
                    <div class="col-md-4 col-md-offset-4">
                        <div class="form-group"><b>&nbsp;Search Vendor Name : </b>
                            <input type="text" name="vendor_name" id="search_vendor" class="form-control" autofocus required style="text-transform:capitalize;">
                            <input type="hidden" name="id_vendor" id="id_vendor">
                        </div>                                                
                    </div>                                                                                       
                    <div class="col-md-2 col-md-offset-5">
                        <div class="form-group">&nbsp;
                            <button type="submit" class="btn btn-success btn-block" name="search" value="1"><span class="glyphicon glyphicon-search"></span> Search   </button>       
                        </div>
                    </div>
                </div>                               
            </form>
            ...
            ?>
            <div class="col-md-12 ">
                <form method="post" name="form2" action="PurchaseController/add_product">
            ...
            <div class="col-md-8 col-md-offset-2">&nbsp;
                <form method="post" name="form6" action="<?php echo site_url('PurchaseController/save_order') ;?>">
                    ...
                    if(count($this->session->userdata('purchase_direct')) > 0){
                        ...
                        { 
                            $s++;
                            //$fetch11=mysqli_fetch_assoc(mysqli_query($conn,"select * from product_desc where id='".$val[0]."'"));

                            //$fetch1=mysqli_fetch_assoc(mysqli_query($conn,"select * from product where product_id='".$fetch11['product_id']."'"));

                            //$brand = mysqli_fetch_assoc(mysqli_query($conn,"select * from brand_master where id='".$fetch1['brand_id']."'"));
...
-->
                            ...
                            <td><a href="<?php echo site_url('PurchaseController/unset_prod/'.$key) ;?>" class="btn btn-danger glyphicon glyphicon-remove btn-sm"></a></td>
                        </tr>
                        <?php   
                        }
                        ?>
                        ...
                        <tr>
                            <td colspan="3">
                                <input class="form-control" type="text" name="receipt" placeholder="Please enter receipt number" required>
                            </td>
                            <td colspan="3">
                                <input class="form-control datepicker" type="text" name="sel_date" placeholder="Please enter date" autocomplete="off" required>
                            </td>
                            <td colspan="4">
                                <button class="btn btn-info" name="order" value="order">Update Purchase</button>
                            </td>
                            <td colspan="3">
                                <button class="btn btn-danger" name="hold_order" value="hold_order">Hold Purchase</button>
                            </td>
                        </tr>
                        <?php }?>
                    </table>
                ...
                <form method="post" name="form4">                                 
                    <div class="col-md-12 "> 
                        <div class="col-md-4 col-md-offset-4">
                            ...
                            <a href="<?php echo site_url('PurchaseController/cancel_deal');?>" class="btn btn-default btn-block" >Cancel</a>
                        </div>
                    </div>                  
                </form>
            </div>
